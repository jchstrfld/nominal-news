...
def main():
    ...
    # Cost-controlled ambiguity review:
    # math-pure = free keep; obvious mixed = free reject; GPT only for competitive ambiguity.
    GPT_LIVE_TARGET = min(EVENT_MAX_CALLS, max(6, top_k + 2))
    AMBIGUOUS_REVIEW_POOL = min(len(pre_ranked), max(top_k * 3, 30))

    for rank_pos, c in enumerate(pre_ranked):
        competitive_ambiguous = rank_pos < AMBIGUOUS_REVIEW_POOL
        ...
        # Ambiguous clusters are never silently accepted.
        should_review = competitive_ambiguous or suspicious
        ...
        if should_review:
            ...
            if cached:
                ...
                c["_gpt_source"] = "cache"
            elif event_calls < GPT_LIVE_TARGET:
            ...
            _print_decision_diag(c, rank_pos, "KEEP", "gpt_single_event")
            kept.append(c)
            continue

        c["eventness_label"] = "UNREVIEWED_LOW_RANK"
        c["_gpt_label"] = "UNREVIEWED_LOW_RANK"
        c["_gpt_source"] = "not_reviewed"
        _print_decision_diag(c, rank_pos, "DEFER", "ambiguous_below_review_pool")
        kept.append(c)
    ...
    if not no_openai and os.getenv("OPENAI_API_KEY"):
        ...
        for c in ranked:
            ...
            if lab in {"SINGLE_EVENT", "SINGLE_EVENT_MATH"}:
                ...
                # Small reserve beyond the normal daily target, only to fill top_k.
                SAFETY_LIVE_LIMIT = min(EVENT_MAX_CALLS, GPT_LIVE_TARGET + 3)

                if event_calls < SAFETY_LIVE_LIMIT:
    ...
    if purity_report:
        print(
            ...
            f"safety_limit={min(EVENT_MAX_CALLS, GPT_LIVE_TARGET + 3)} "
